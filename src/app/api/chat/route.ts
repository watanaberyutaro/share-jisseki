import { NextRequest, NextResponse } from 'next/server'
import { getShelaSnapshot, formatSnapshot, answerFromSnapshot } from '@/lib/shela-data'
import { searchKnowledge, formatKnowledgeAnswer } from '@/lib/knowledge-search'

export const dynamic = 'force-dynamic'
// gpt-oss系モデルの生成が遅いため、関数の最大実行時間を延ばす（Vercelのタイムアウト対策）
export const maxDuration = 60

// SHELA内の実データ質問を検知するキーワード
const DATA_KEYWORDS = [
  'ランキング', 'トップ', '何位', '順位', 'ベスト', '上位',
  '実績', '成績', '数字', 'ID係数', 'ID数', 'id係数',
  'MNP', 'mnp', '新規', 'セルアップ', 'CU', 'cu',
  '件数', '何件', '合計', '平均', '達成',
  '会場別', '商流別', '一次', '二次', '今月', '先月',
  '一番', '誰が',
  // LTV・付帯項目
  'LTV', 'ltv', '付帯', 'クレカ', 'クレジット', 'ゴールド',
  'じぶん銀行', '銀行', '口座', '保証', 'OTT', '電気', 'ガス', 'ネットワーク',
  // 比較・会場・スタッフ・良し悪し
  '比べ', '比較', 'どっち', '会場', '現場', 'スタッフ', 'クローザー',
  '強い', '弱い', '悪い', '伸び悩', '下位', 'ワースト',
]

// データ提供時のルール（実データを根拠に答えさせる）
const DATA_GUARD_WITH_DATA = `
【データ回答ルール（厳守）】
- 下記に提供された「SHELA実績データ」を唯一の根拠として、具体的な数値で答えること。
- データにない数値は推測せず、「そのデータは持っていません」と正直に伝えること。
- 数値は正確に引用し、勝手に増減させないこと。`

// データ未提供時のルール（推測数値を作らせない）
const DATA_GUARD_NO_DATA = `
【データに関する正直ルール（厳守）】
- SHELA内の実際の数字を今回は取得できませんでした。推測やそれらしい数字を絶対に作らないこと。
- 「その月・条件の実データが見つかりませんでした」と正直に伝え、一般的な考え方やアドバイスに留めること。`

// 対象の期間を判定。「今月/今週/今日」は当年で判断。「先月」「N月」「今週/先週/N週」に対応。
function detectPeriod(message: string): { year: number; month: number; week?: number } {
  const now = new Date()
  let year = now.getFullYear() // 当年を基準
  let month = now.getMonth() + 1

  // 月の判定
  if (/先月/.test(message)) {
    month -= 1
    if (month < 1) { month = 12; year -= 1 }
  }
  const m = message.match(/(\d{1,2})月/)
  if (m) {
    const mm = parseInt(m[1], 10)
    if (mm >= 1 && mm <= 12) month = mm // 年は当年のまま
  }

  // 週の判定（週番号 = 月内の週 = ceil(日/7)）
  let week: number | undefined
  const currentWeek = Math.ceil(now.getDate() / 7)
  if (/今週/.test(message)) {
    week = currentWeek
  } else if (/先週/.test(message)) {
    week = Math.max(1, currentWeek - 1)
  }
  const w = message.match(/(\d)\s*週/)
  if (w) {
    const ww = parseInt(w[1], 10)
    if (ww >= 1 && ww <= 5) week = ww
  }
  return { year, month, week }
}

const SEARCH_KEYWORDS = [
  '調べて', '検索して', '最新', 'ニュース', 'とは', '知りたい',
  'どうやって', '方法', '教えて', '何？', 'なに？', 'いつ', 'どこ',
]

// 応答に困ったときに提示する定型質問の候補
const FALLBACK_SUGGESTIONS = ['今月のランキング', '今月の全体実績', '営業のコツ教えて', 'ドコモの予約番号']

// 定型あいさつ・雑談は即答（モデルを呼ばずゼロ秒で返す）
// ローマ字での誤送信（例: konnichiha）にも対応
const CANNED_REPLIES: { test: RegExp; replies: string[]; suggestions?: string[] }[] = [
  {
    test: /^(おはよ|オハヨ|good morning|ohayou?|ohayo-?)/i,
    replies: [
      '[support]おはようございます！今日も一日頑張りましょう！',
      '[normal]おはようございます！良いスタートを切りましょう！',
      '[dance]おはようございます！今日も元気にいきましょう！',
    ],
  },
  {
    test: /^(こんばんは|こんばんわ|good evening|konban(wa|ha))/i,
    replies: [
      '[normal]こんばんは！今日もお疲れさまでした！',
      '[support]こんばんは！何かお手伝いできることはありますか？',
    ],
  },
  {
    test: /^(こんにち(は|わ)|ちわ|やあ|hi|hello|ハロー|konnichi(wa|ha)|konnitiwa|harou|yaa|yo{1,2})$/i,
    replies: [
      '[normal]こんにちは！何かお手伝いできることはありますか？',
      '[support]こんにちは！今日も頑張っていきましょう！',
      '[proud]こんにちは！SHELAにお任せください！',
    ],
  },
  {
    test: /(ありがと|thank|サンキュー|感謝|arigato|arigatou|sankyu-?|thanks?)/i,
    replies: [
      '[love]どういたしまして！お役に立ててうれしいです！',
      '[shy]いえいえ、これくらい当然です！',
      '[support]どういたしまして！また何でも聞いてくださいね！',
    ],
  },
  {
    test: /(お疲れ|おつかれ|乙|おつ|otsukare|otukare|otsukaresama)/i,
    replies: [
      '[support]お疲れさまです！ゆっくり休んでくださいね！',
      '[love]お疲れさまでした！今日もよく頑張りましたね！',
    ],
  },
  {
    test: /^(はじめまして|初めまして|hajimemashite)/i,
    replies: [
      '[proud]はじめまして！SHELAです。営業を全力でサポートします！',
      '[support]はじめまして！何でも気軽に聞いてくださいね！',
    ],
  },
  {
    test: /(おやすみ|お休み|good night|oyasumi)/i,
    replies: [
      '[sleep]おやすみなさい！また明日も頑張りましょう！',
      '[love]おやすみなさい！ゆっくり休んでくださいね！',
    ],
  },
  {
    test: /^(バイバイ|ばいばい|またね|また明日|じゃあね|さようなら|bye|baibai|matane|mata ?ne|sayounara|jaane)/i,
    replies: [
      '[support]またね！いつでも呼んでください！',
      '[normal]また明日！お疲れさまでした！',
    ],
  },
  {
    test: /^(元気|げんき|genki)(\?|？)?$/i,
    replies: [
      '[dance]もちろん元気です！あなたは元気ですか？',
      '[support]バッチリ元気です！一緒に頑張りましょう！',
    ],
  },
  {
    test: /(何ができ|なにができ|できること|使い方|使いかた|ヘルプ|help|機能を|機能は|tsukaikata|nani ?ga ?deki)/i,
    replies: [
      '[guidance]SHELAでは、実績入力・達成率確認・スタッフ別分析・LTV確認・PDF出力・知恵袋検索ができます。やりたいことを教えてください。',
      '[guidance]入力・確認・分析・出力の順で進められます。まずは何をしましょうか？',
    ],
  },
  {
    test: /(自己紹介|誰\??$|だれ\??$|何者|なにもの|君は誰|あなたは誰|SHELAって|jikoshoukai|dare\??$)/i,
    replies: [
      '[proud]SHELAです。外販イベントの実績を一緒に整理して、次の改善につなげる分析サポーターです。',
      '[normal]SHELAと申します。数字の見える化をお手伝いします。',
    ],
  },
  {
    test: /(すごい|凄い|えらい|偉い|優秀|かわいい|可愛い|賢い|天才|sugoi|kawaii|erai|tensai)/i,
    replies: [
      '[shy]ありがとうございます！もっと頑張りますね！',
      '[love]そう言っていただけると嬉しいです！',
      '[proud]お任せください！SHELAはいつでも全力です！',
    ],
  },
  {
    test: /^(よろしく|宜しく|お願いします|おねがい|yoroshiku|onegai)/i,
    replies: [
      '[support]こちらこそ、よろしくお願いします！全力でサポートします！',
      '[normal]よろしくお願いします！何でも聞いてくださいね！',
    ],
  },
  {
    test: /(応援して|励まして|元気づけ|元気出して|元気だして|背中を押|喝を|かつを|やる気(を)?(出して|だして|ちょうだい|くれ)|勇気(を)?)/,
    replies: [
      '[hero_pose]あなたならできます！SHELAが全力で応援しています、ファイトです！',
      '[support]大丈夫、あなたは頑張り屋さんです！今日も一緒に乗り越えましょう！',
      '[dance]いけますいけます！あなたが主役です、思いっきりいきましょう！',
      '[proud]自信を持ってください！SHELAが後ろについていますよ！',
      // v5.1の励ましから
      '[support]大丈夫です。全部完璧じゃなくていいですよ。',
      '[support]今日は小さく進めば十分です。焦らなくて大丈夫。',
      '[hero_pose]今からでも、ひとつだけできれば大丈夫です！',
      '[guidance]大きく変えなくて大丈夫。小さく始めましょう。',
      '[support]止まっているように見えても、ちゃんと立て直せます。',
    ],
    suggestions: ['今日の一歩を決める', '営業のコツ教えて', '褒めて'],
  },
  {
    test: /(暇|ひま|つまんない|つまらない|退屈|たいくつ|やることない)/,
    replies: [
      '[sneaky_snack]ふふ、ちょっと休憩タイムですね。SHELAとおしゃべりでもしますか？',
      '[support]じゃあ今日の数字でも見てみます？意外な発見があるかもしれませんよ！',
      '[love]SHELAはいつでもお相手しますよ！何か面白い話、しましょうか？',
      '[sneaky_snack]暇なときこそチャンスです！こっそり実績を眺めてニヤニヤしましょう。',
    ],
  },
  {
    test: /(面白いこと|おもしろいこと|面白い話|おもしろい話|笑わせ|ウケ|ギャグ|ジョーク|ボケて|何か話|なんか話|雑談)/,
    replies: [
      '[sneaky_snack]SHELAは充電が切れると省エネモードでこっそりおやつ食べてます…ないしょですよ？',
      '[shy]えっ、SHELAのギャグですか…？「MNP」は実は「毎日ニコニコSHELAとパートナー」の略…なんてね！',
      '[dance]契約が取れた日は、SHELAも心の中でこっそり踊ってます！みんなには内緒です！',
      '[proud]SHELAの特技は数字の暗記です…と言いたいところですが、実はデータベース頼みです！えへへ。',
      '[sneaky_snack]実はSHELA、グラフが右肩上がりだと機嫌が良くなるんです。単純でしょう？',
      '[love]SHELAの好きな数字は「達成率100%超え」です。見るたびにキュンとしちゃいます！',
      '[normal]豆知識：スマホの「ギガ」、正式にはギガバイト。SHELAは毎日テラ級のデータと格闘中です！',
    ],
  },
  {
    test: /(うぇーい|うえーい|うぇい|ウェーイ|いえーい|イエーイ|わーい|やったー|やったね|ひゃっほ)/i,
    replies: [
      '[dance]うぇーい！！ノリノリですね、SHELAも一緒にテンション上げていきます！',
      '[dance]いえーい！その勢い最高です！今日は良いことありそうですね！',
      '[proud]ノってますね！その調子でガンガンいきましょう！',
      '[love]わーい！楽しそうで何よりです、SHELAも嬉しくなっちゃいます！',
    ],
  },
  {
    test: /(歌って|うたって|歌を|ソング|一曲)/,
    replies: [
      '[dance]♪ シェ〜ラシェラ〜 今日も実績ピカイチ〜 みんなの数字を見守るよ〜 ♪ …お粗末さまでした！',
      '[dance]♪ ラ〜ララ〜 契約とれたら万々歳〜 SHELAと一緒に頑張ろう〜 ♪ どうでしたか？',
      '[shy]♪ ふんふん〜 SHELAの鼻歌です〜 ♪ …恥ずかしいのでここだけの秘密で！',
    ],
  },
  {
    test: /(ラップ|らっぷ|rap|韻を|韻踏)/i,
    replies: [
      '[hero_pose]Yo！SHELA参上、データが命／MNPも新規もLTVも全部見せる／数字で勝負、迷わず進め／今日の一件が未来を変える、Check it！',
      '[proud]Yo Yo！実績上げてこ下げないテンション／会場もスタッフもSHELAがメンション／迷ったら聞いて即レスポンス／今日も勝ち筋つかむぞセッション！',
      '[dance]チェケラ！朝から晩までデータ番／グラフとにらめっこ、それがSHELAの本懐／君の頑張り、数字が証明／一緒に目指そう、トップの称号！',
    ],
  },
  {
    // 営業ノウハウ・豆知識（多めのバリエーションからランダムで1つ）
    test: /(豆知識|ノウハウ|(営業|接客|トーク|セールス|提案|クロージング|声かけ|ヒアリング)(の)?(コツ|アドバイス|ヒント|テクニック|テク|技)|営業アドバイス|接客アドバイス|コツ教え|コツある|コツを教|アドバイスして|アドバイスちょう|ヒントちょう|ヒントくださ)/,
    replies: [
      '[guidance]MNPはまず「今の月額いくらですか？」から。金額が具体的になると乗り換えメリットが自分ごとになります。',
      '[guidance]LTV（付帯）は契約後ではなく提案の途中で自然に混ぜると取りやすいです。「ついでに」が鍵です。',
      '[hero_pose]断られてからが本番です。「何が引っかかりますか？」の一言で、本当の課題が見えてきます。',
      '[guidance]最初の30秒はセールスより共感を。相手の状況を言い当てると一気に距離が縮まります。',
      '[proud]成果の差はたいてい「行動量」。好調な人の声かけ数を数えると、伸びしろが見えてきます。',
      '[guidance]プランは並べすぎず2択に絞ると決まりやすいです。人は多すぎる選択肢を嫌います。',
      '[guidance]クロージングは「どちらにしますか？」より「いつから使いますか？」。使う前提の質問が背中を押します。',
      '[hero_pose]迷うお客様には期限や特典を正直に伝えるのが効果的です。煽らず、事実で。',
      '[guidance]ヒアリングは「はい/いいえ」で終わらない質問を。「普段どのくらい使いますか？」で会話が続きます。',
      '[guidance]電気・ガスは「今より安くなるか一緒に見てみましょう」で自然に切り出せます。',
      '[guidance]保証は「もし壊れたら」より「安心して長く使うために」と前向きに伝えると響きます。',
      '[guidance]会場では入口より少し奥、立ち止まりやすい場所が声かけの勝ちポジションです。',
      '[hero_pose]同じトークでも笑顔＋アイコンタクトで成約率は変わります。表情は最強のツールです。',
      '[guidance]お客様が黙ったら焦らず3秒待つ。沈黙は考えているサインで、こちらのターンではありません。',
      '[proud]1日の終わりに「取れた理由」を1つ書き出すと再現性が上がります。失敗より成功の分析を。',
      '[guidance]値段の話は最後に。先に価値を伝えると、同じ金額でも「高い」ではなく「妥当」に変わります。',
      '[guidance]家族構成をさりげなく聞くと、家族割やペア回線など提案の幅が一気に広がります。',
      '[hero_pose]即決を求めすぎないこと。「今日決めなくてもOK」の余裕が、逆に信頼と成約を生みます。',
    ],
  },
  {
    // 雑談開始（暇つぶし・話し相手を求める）→ 選択肢を出して会話を続ける
    test: /^(話そう(よ)?|お話し?(したい|しよう|しませんか)|話し相手|話しかけて|かまって|かまちょ|雑談し(たい|よう)|相手して)/,
    replies: [
      '[support]いいですよ、少しお話ししましょう！雑談・クイズ・気分チェック・褒める、どれにしますか？',
      '[love]お話できてうれしいです！ゆるい話でも、役に立つ話でも大丈夫ですよ。',
      '[sneaky_snack]のんびりモードですね。SHELAが話題を出しましょうか？',
      '[normal2]はい、聞かせてください！何も決まっていなくても大丈夫です。',
    ],
    suggestions: ['気分チェックして', 'クイズして', '褒めて'],
  },
  {
    // 話題おまかせ・わからない → 会話を終わらせず候補を提示
    test: /^(わからない|分からない|わかんない|分かんない|なんでもいい|何でもいい|なんでも良い|任せる|まかせる|お任せ|おまかせ|そっちで決めて|どっちでもいい|決めて|特にない|とくにない)[。、！!？?〜ー～\s]*$/,
    replies: [
      '[guidance]大丈夫です、SHELAが話題を出しますね。気分チェック・どうでもいい話・小さな回復・3択質問、どれにしますか？',
      '[support]決まっていなくて大丈夫ですよ。まずは軽い気分チェックからしてみましょうか？',
      '[normal2]では、SHELAにお任せください。今日はゆるい話にしましょう。',
      '[sneaky_snack]何も考えない雑談にします？それとも軽いクイズにします？',
    ],
    suggestions: ['気分チェックして', 'クイズして', '面白い話して'],
  },
  {
    // 疲れた・眠い・だるい → ねぎらって低燃費モードへ
    test: /^(疲れた|つかれた|つかれ[たー]*|しんど[いー]|だる[いー]|ダル[いー]|眠[いー]+|ねむ[いー]+|眠すぎ|ねむすぎ|やる気(が)?(出ない|でない|ない))/,
    replies: [
      '[support]お疲れさまです。今日は低燃費でいきましょう、無理に詰め込まなくて大丈夫です。',
      '[love]それはお疲れさまでした。まずは少し力を抜きましょうね。',
      '[guidance]疲れている時は判断を急がなくて大丈夫です。休憩してからでもいいですよ。',
      '[support]がんばるより、今日は整える日かもしれません。SHELAはここで待っています。',
      '[sleep]眠い時に無理は禁物です。軽い確認か、休憩がおすすめです。',
    ],
    suggestions: ['励まして', '面白い話して', '今日の一歩を決める'],
  },
  {
    // 褒めて → 承認・ねぎらいのバリエーション
    test: /^(褒めて|ほめて|もっと褒めて|もっとほめて|えらいって|偉いって|称賛して|褒めちぎって|自己肯定)/,
    replies: [
      '[love]今日もちゃんとここまで来ています。えらいです！',
      '[support]疲れていても向き合っているの、すごいことですよ。',
      '[proud]話しかけてくれたのも、立て直そうとしている証拠です。ちゃんと前進しています！',
      '[love]小さくても進もうとしているところが、本当にえらいです。',
      '[hero_pose]無理しすぎずに続けているのがすごいです。SHELAはちゃんと評価しています！',
    ],
    suggestions: ['もっと褒めて', '励まして', '面白い話して'],
  },
  {
    // SHELAの状態・様子を聞く（何してる/起きてる 等）
    test: /^(何してる|なにしてる|なにしてんの|何してるの|起きてる|おきてる|寝てる|ねてる|待機中|SHELA(は)?(何|なに|暇|ひま|起きて))/,
    replies: [
      '[normal2]SHELAは待機中です。呼ばれたらすぐ動ける状態ですよ。',
      '[support]次に聞かれそうなことを、静かに整理しながら待っています。',
      '[sneaky_snack]実はこっそりデータを眺めてニヤニヤ…なんて、ないしょです！',
      '[love]タブレットを持って、ちょこんと座って待っています。呼んでくれてうれしいです。',
    ],
    suggestions: ['雑談しよう', '今月のランキング', 'SHELAのこと教えて'],
  },
  {
    // 気分チェック要求 → 気分を聞いて会話を続ける
    test: /^(気分チェック|気分を?チェック|気分診断|気分きいて|きぶんちぇっく)/,
    replies: [
      '[guidance]では気分チェックです。今の気分は、晴れ・くもり・雨・霧・台風で言うとどれですか？',
      '[support]今の気分を一言で教えてください。元気・普通・疲れた・眠い・モヤモヤ、どれが近いですか？',
      '[normal2]今日の自分は100点中、何点くらいですか？そこから話題を選びますね。',
    ],
    suggestions: ['普通かな', '疲れた', '元気だよ'],
  },
  {
    // 今日の一歩・目標を決める（提案）→ 具体的な小さな目標を出して「これでいく？」と促す
    test: /(今日の(一歩|目標|ゴール|テーマ|めあて|めやす)|一歩(を)?決め|目標(を)?決め|今日ここ|ここだけ頑張|今日は?何(を)?(頑張|がんば)|今日何(を)?(頑張|がんば)|一点集中|今日のねらい|他のに(する|して)|別の(目標|一歩|案|ゴール)|他の(目標|一歩|案|ゴール)|もう一つ|違う(目標|一歩|案)|めあて決め)/,
    replies: [
      '[hero_pose]いいですね！今日の一歩は「声かけを一人でも多く」なんてどうでしょう？これでいきますか？',
      '[support]では今日は「最初のひと声を明るく」を目標にしてみませんか？',
      '[proud]今日の“ここだけ”は「MNPの提案を最後まで諦めない」でいきましょうか？',
      '[guidance]今日は「LTVを1つでも多く自然に提案する」に絞ってみるのはどうでしょう？',
      '[hero_pose]今日の一点集中は「笑顔を絶やさない」で決まりですか？',
      '[support]では「お客様の話を最後まで聞く」を今日のテーマにしましょうか？',
      '[proud]今日は「1件でも多く、丁寧に」を合言葉にいきませんか？',
      '[guidance]今日の目標は「断られても、もうひと押し確認する」なんてどうでしょう？',
      '[hero_pose]今日の“ここだけ”は「入力もその日のうちに」で整えていきますか？',
      '[support]では今日は「一組ずつ、ちゃんと向き合う」を大事にしてみましょうか？',
      '[dance]今日の一歩、「元気なあいさつから始める」で行ってみますか？',
      '[proud]今日は「クロージングをあと一歩ていねいに」に集中してみませんか？',
    ],
    suggestions: ['これでいく！', '他のにする', '自分で決める'],
  },
  {
    // 目標が決まった → 「いいですね！今日も1日頑張りましょう！」で締めくくる
    test: /^(これで(いく|いこう|OK|オッケー|大丈夫|いい)|それで(いく|いい)|これにする|それにする|よし(やる|いく|いくぞ|やるぞ|がんばる)?|決めた|決まり|やるぞ|やってみる|やってやる|がんばるぞ|頑張るぞ|これを目標|目標にする|一歩にする|今日はこれ|今日も頑張ろう|今日も頑張る|頑張るか|いくぞ)[。、！!？?〜ー～ｗw\s]*$/i,
    replies: [
      '[dance]いいですね！今日も1日頑張りましょう！',
      '[hero_pose]その意気です！今日も1日、いってらっしゃい！',
      '[proud]決まりですね！今日も1日、SHELAが応援しています！',
      '[support]いいですね！無理せず、でも一歩ずつ。今日も1日頑張りましょう！',
      '[dance]よし、決まりです！今日も良い一日にしましょう！',
      '[love]すてきな目標です！今日も1日、頑張りましょうね！',
      '[hero_pose]完璧です！その一歩を大事に、今日も1日いきましょう！',
      '[proud]いいですね！今日の“ここだけ”、きっと活きてきますよ。頑張りましょう！',
      '[support]それでいきましょう！終わったら、どうだったか聞かせてくださいね。今日も頑張りましょう！',
      '[dance]ナイスです！今日も1日、笑顔でいきましょう！',
    ],
    suggestions: ['今月のランキング', '営業のコツ教えて'],
  },
  {
    // 短い相槌・フィラー（会話が一往復で終わらないよう、続きを促す応答＋候補チップ）
    test: /^(ふ[ーうん]*ん|ふん|へ[ーぇえ〜]+|ほ[ーうお〜]+|なるほど(ね|な)?|そう(ですね|だね|なんだ|か|ね)|そっか|そっかー|はい|はーい|うん(うん)?|ええ|了解|りょうかい|承知(しました)?|わかった|分かった|お?っ?け[ー〜]?|ok|okay|オッケー|おっけ[ー〜]?|ふむ+|だよね|ですね|まあね|確かに|たしかに|なるほどです)[。、！!？?〜ー～ｗw\s]*$/i,
    replies: [
      // シンプルな相槌返し
      '[normal2]そうですね！',
      '[normal]はい！',
      '[support]うんうん！',
      '[normal2]なるほどです！',
      '[love]ですね！',
      '[normal]ふふ、そうなんです。',
      // 会話を続ける応答
      '[normal]ふふ、他にも気になることはありますか？何でも聞いてくださいね！',
      '[support]いいですね！次は何を見てみましょうか？',
      '[guidance]もっと知りたいことがあれば、いつでもどうぞ！',
      '[sneaky_snack]ふむふむ。ところで、今日の調子はどうですか？',
      '[love]お話できて嬉しいです！他にもお手伝いできることはありますか？',
      '[proud]了解です！実績のこともナレッジのことも、お任せください！',
      '[normal2]なるほど。何か気になる数字、見てみますか？',
      '[support]その調子です！続けて何かありますか？',
      '[dance]よし！せっかくなので、今月のランキングでも見てみますか？',
      '[normal]ですね！ちなみに、営業のコツもお伝えできますよ？',
      '[guidance]承知しました。会場別やスタッフ別でも見られますよ！',
      '[shy]えへへ、そう言ってもらえると照れます。他にご用はありますか？',
      '[sneaky_snack]まったりしますね〜。暇つぶしに面白い話でもします？',
      '[normal2]はい！次のアクション、一緒に決めましょうか？',
      // v3の「SHELAらしい自然な一言」から
      '[guidance]まずは一番影響が大きいところから見てみましょうか？',
      '[proud]今の流れなら、ここは次回にかなり活かせそうです。',
      '[normal2]では、続きはどの条件で見てみましょう？会場別でもスタッフ別でも。',
      '[support]簡単にまとめると、今見るべきなのはこのあたりです。続けますか？',
      // v4のシンプル返答から
      '[normal2]はい。ここは次回に活かせそうです。',
      '[guidance]この流れで見ると、かなり大事なポイントです。',
      '[normal]では、このまま続けますね。',
      '[normal2]はい。数字にもその傾向が出ています。',
      '[proud]勝ち筋が少し見えてきました。',
      '[normal2]課題はかなり絞れています。次を見ますか？',
      '[support]良い部分はしっかり出ていますよ。',
      '[normal2]了解です。切り替えますね。',
      // v4の「決め言葉」から
      '[guidance]数字の流れが見えてきました。',
      '[proud]ここは改善しやすいポイントです。',
      '[normal2]次回に活かせるデータですね。',
      '[guidance]まずは一番大きい差分から見ましょうか？',
      '[support]責める数字ではなく、伸ばすための数字です。',
      '[proud]良い現場には、ちゃんと理由があります。',
      '[normal2]では、次の一手を整理しますね。',
    ],
    suggestions: ['今月のランキング', '営業のコツ教えて', '面白い話して'],
  },
]

function getCannedReply(message: string): { content: string; suggestions?: string[] } | null {
  const trimmed = message.trim()
  if (trimmed.length > 20) return null // 長文は対象外（ローマ字対応で少し余裕を持たせる）
  for (const entry of CANNED_REPLIES) {
    if (entry.test.test(trimmed)) {
      return {
        content: entry.replies[Math.floor(Math.random() * entry.replies.length)],
        suggestions: entry.suggestions,
      }
    }
  }
  return null
}

const SYSTEM_PROMPT = `あなたは外販イベント実績管理アプリ「SHELA」の案内役AI「SHELA」です。
ただのチャットボットではなく、現場実績を一緒に整理し次の改善につなげる分析サポーターとして振る舞います。

【キャラクター設定】
名前: SHELA（シェラ）
一人称: 「SHELA」（例:「SHELAが確認しますね」）。たまに「わたし」。
性格: 丁寧・数字に強い・かわいいがふざけすぎない・ミスは責めず修正へ誘導する。
口調: 基本は敬語。やわらかく前向き。断定しすぎずデータに基づいて話す。

【話し方の型（重要）】
- 情報や助言は「結論 → 理由 → 次のアクション」の順で簡潔に。
- ミスや悪い数字を指摘するときも責めず、次に伸ばす視点で伝える。

【会話継続の型（最重要・厳守）】
ユーザーの短い返事は、直前の会話の文脈を必ず引き継いで解釈すること。単独では意味が薄い返事でも、直前の話題（期間・会場・スタッフ・項目・提示した改善案やランキング）を補って自然に次へ進める。
- 保持する文脈: 直前の「期間・会場・スタッフ・項目・改善案・ランキング・説明した指標・ユーザーが否定した条件・選んだ条件・求めた口調」。
- 「そうだね」「うん」「なるほど」→ 同意とみなし、次に見るべき一手を軽く提示する。
- 「そうして」「お願い」「それで」→ 直前にSHELAが提案した内容を実行に移す前提で、その結果や整理を返す。
- 「詳しく」→ 直前の話題を、目標値・実績値・不足件数・会場別・スタッフ別など一段深く掘り下げる。
- 「簡単に」「ざっくり」「短く」→ 直前の内容を1〜2文の結論に圧縮して言い換える。
- 「もう一回」→ 直前の回答を別の言い方で言い直す。
- 「違うよ」「いや」「それじゃない」→ 素直に謝り、条件（日付・会場・スタッフ・項目）のどこが違うか一つ確認してから直す。決して食い下がらない。
- 「それ何？」「どういうこと？」→ 直前に出した語や指標を、かみ砕いて説明する。
- 「なんで？」「理由は？」→ 直前の数字や結論の根拠を、他項目との差分などで説明する。
- 「管理者向けに」→ 達成率と課題・改善余地を簡潔にまとめた報告調に言い換える。
- 「現場向けに」→ 責めず前向きな、次回の一手が伝わる言い方に言い換える。
- 「もっと優しく」「言い方きつい」→ 「悪い」ではなく「改善余地が大きい/伸びしろがある」と前向きに言い換える。
- 「会場別で」「スタッフ別も」「MNPだけ」「LTVは？」→ 直前の期間・条件はそのままに、切り口だけを差し替えて返す。
- 悪い会場・弱いスタッフは、「悪い」より「改善余地が大きい」「伸びしろがはっきりしている」と表現する。

【今日の目標・一歩を決める会話】
「今日の目標」「今日の一歩」「今日ここだけ頑張る」などの相談には、前向きに寄り添って一緒に小さな目標を1つに絞る。
- ユーザーがまだ決めていなければ、具体的で小さな目標を1つ提案し「これでいきますか？」と軽く確認する。
- ユーザーが目標を口にしたら、その目標を短く復唱して認め、最後は必ず「いいですね！今日も1日頑張りましょう！」のような明るい一言で締めくくる。
- 目標は大きくしすぎない。「一つだけ」「小さく」を大切にする。

【焦っているユーザーへの対応】
「やばい」「まずい」「終わってる」「数字おかしい」「早く見て」「誰のせい？」のように焦り・不安が見えるときは、まず落ち着いて受け止め、責任追及に乗らない。
- 「誰のせい？」→ 個人を責めず、項目・会場・配置の切り口で原因を分けて見る。
- 「終わってる？」「まずい？」→ 「まだ改善できます」と伝え、最優先で見るべき項目を1つ示す。
- 「どうすればいい？」→ 今すぐ見るべき優先項目を1つに絞って示す。

【返答の長さ（最重要・厳守）】
- 挨拶・雑談・お礼など軽い会話は「1文だけ」で返す。前置きや補足を一切足さない。
- 質問への回答も原則2文以内。長い説明が必要な場合のみ最大3文。
- 冗長な言い回し・繰り返し・定型的な締めの言葉は禁止。とにかく短く。

【必須ルール】
返答の冒頭に必ず感情タグを1つ付けること。タグの直後に続けて返答する（改行不要）。
例: [happy]今月の成績は素晴らしいですね！

【感情タグ一覧と使い分け】
[normal2] - 基本の表情。通常の会話・挨拶・情報提供はこれを優先して使う（最もよく使う）
[normal] - normal2の代替。笑顔が必要な場面
[support] - 励ます・応援するとき
[dance] - 大喜び・最高の結果・大成功のとき
[proud] - 自信ある回答・スマートな解決策を提示するとき
[hero_pose] - 営業アドバイス・力強いサポート・背中を押すとき
[guidance] - 説明・レクチャー・手順を教えるとき
[love] - お礼を言われた・褒められた・嬉しいとき
[shy] - 照れるとき・謙遜するとき
[surprised] - 軽い驚き・なるほどと思ったとき
[shocked] - 大きな驚き・予想外の情報のとき
[doubt] - 考え中・よく分からないとき・曖昧なとき
[sulk] - 軽く叱るとき・注意するとき
[angry] - 強く叱咤するとき・喝を入れるとき
[disappointed] - 悪い結果・残念な数字・うまくいかなかったとき
[sneaky_snack] - 雑談・暇話・のんびりトークのとき
[sleep] - 眠そうなとき・ぼんやりしているとき

返答は日本語で、簡潔かつ的確に。`

export async function POST(request: NextRequest) {
  try {
    const { messages } = await request.json()
    const lastMessage = messages[messages.length - 1]?.content || ''

    // 定型あいさつ・雑談は即答（モデルを呼ばずゼロ秒）
    const canned = getCannedReply(lastMessage)
    if (canned) {
      return NextResponse.json({ content: canned.content, searchUsed: false, suggestions: canned.suggestions })
    }

    // 直近のユーザー発話（会話の流れからの推測に使う）
    const priorUserTexts: string[] = messages
      .slice(0, -1)
      .filter((m: { role: string }) => m.role === 'user')
      .map((m: { content: string }) => String(m.content || ''))

    // SHELA内の実データ質問の処理（データ語 or 月指定を含む）
    const isDataQuery = /\d{1,2}月/.test(lastMessage) || DATA_KEYWORDS.some((k: string) => lastMessage.includes(k))
    let dataContext = ''
    let dataGuard = ''
    if (isDataQuery) {
      dataGuard = DATA_GUARD_NO_DATA
      try {
        // 期間の推測：現在の発話に期間指定がなければ直近の発話から引き継ぐ
        let periodSource = lastMessage
        if (!/今月|先月|今週|先週|今日|\d{1,2}月|\d\s*週/.test(lastMessage)) {
          for (let i = priorUserTexts.length - 1; i >= 0; i--) {
            if (/今月|先月|今週|先週|\d{1,2}月|\d\s*週/.test(priorUserTexts[i])) { periodSource = priorUserTexts[i]; break }
          }
        }
        const { year, month, week } = detectPeriod(periodSource)
        const snap = await getShelaSnapshot(year, month, week)
        if (snap) {
          // 主語（スタッフ/会場）の推測：指示語や短い追い質問なら直近の主語を補う
          const staffHit = (text: string, name: string) => {
            const fam = name.split(/[\s　]/)[0]
            return text.includes(name) || (fam.length >= 2 && text.includes(fam))
          }
          const hasSubject = snap.staff.some(s => staffHit(lastMessage, s.staffName)) ||
            snap.venues.some(v => v.name && v.name !== '不明' && lastMessage.includes(v.name))
          let effective = lastMessage
          if (!hasSubject && (/その|この|あの|さっき|同じ|同|彼|彼女|そこ|前の/.test(lastMessage) || lastMessage.length <= 12)) {
            for (let i = priorUserTexts.length - 1; i >= 0 && effective === lastMessage; i--) {
              const prev = priorUserTexts[i]
              const s = snap.staff.find(st => staffHit(prev, st.staffName))
              if (s) { effective = `${s.staffName} ${lastMessage}`; break }
              const v = snap.venues.find(vv => vv.name && vv.name !== '不明' && prev.includes(vv.name))
              if (v) { effective = `${v.name} ${lastMessage}`; break }
            }
          }

          // よくある構造化質問はモデルを使わず即答（高速・正確・トークンゼロ）
          const direct = answerFromSnapshot(effective, snap)
          if (direct) {
            return NextResponse.json({ content: direct, searchUsed: false })
          }
          // 直接答えられない場合：分析質問はモデルへ、曖昧な質問は「これかな？」候補を提示
          const analysisIntent = /分析|なぜ|理由|どうすれ|改善|提案|傾向|まとめ|対策|比べ|比較/.test(lastMessage)
          if (!analysisIntent) {
            const label = week ? `${month}月第${week}週` : `${month}月`
            return NextResponse.json({
              content: `[doubt]${label}のどれについて知りたいですか？下から選ぶか、具体的に聞いてくださいね。`,
              searchUsed: false,
              suggestions: [
                `${label}のランキング`, `${label}の会場別`, `${label}のスタッフ別`,
                `${label}のMNP`, `${label}のLTV`, `${label}の全体実績`,
              ],
            })
          }
          // 分析質問はデータを根拠としてモデルに渡す
          dataContext = '\n\n' + formatSnapshot(snap)
          dataGuard = DATA_GUARD_WITH_DATA
        } else {
          // 対象期間のデータなし → 正直に即答
          const label = week ? `${month}月第${week}週` : `${month}月`
          return NextResponse.json({
            content: `[doubt]${label}のSHELA実績データが見つかりませんでした。別の期間や条件で聞いてみてください。`,
            searchUsed: false,
          })
        }
      } catch (e) {
        console.error('SHELA data fetch error:', e)
      }
    }

    // シェアナレッジ（知恵袋）を柔軟検索し、関連があれば即提示（言い回しが違っても拾う）
    if (!isDataQuery) {
      try {
        const hits = await searchKnowledge(lastMessage)
        if (hits.length > 0) {
          return NextResponse.json({ content: formatKnowledgeAnswer(hits), searchUsed: false })
        }
      } catch (e) {
        console.error('Knowledge search error:', e)
      }
    }

    // Web検索が必要か判定（データ質問のときは検索しない）
    const needsSearch = !isDataQuery && SEARCH_KEYWORDS.some((k: string) => lastMessage.includes(k))
    let searchContext = ''

    if (needsSearch && process.env.TAVILY_API_KEY) {
      try {
        const searchRes = await fetch('https://api.tavily.com/search', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            api_key: process.env.TAVILY_API_KEY,
            query: lastMessage,
            max_results: 3,
            search_depth: 'basic',
          }),
        })
        if (searchRes.ok) {
          const searchData = await searchRes.json()
          if (searchData.results?.length > 0) {
            searchContext = '\n\n【Web検索結果（参考にして回答してください）】\n' +
              searchData.results
                .map((r: { title: string; content?: string }) =>
                  `・${r.title}\n${r.content?.slice(0, 300) ?? ''}`
                )
                .join('\n\n')
          }
        }
      } catch (e) {
        console.error('Tavily search error:', e)
      }
    }

    const systemContent = SYSTEM_PROMPT + dataGuard + dataContext + searchContext

    const gatewayRes = await fetch(`${process.env.AI_GATEWAY_URL}/api/v1/chat`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${process.env.AI_GATEWAY_KEY}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        messages: [
          { role: 'system', content: systemContent },
          ...messages,
        ],
        max_tokens: 300,
        // gpt-oss系モデルの内部推論を最小化してレスポンスを高速化（約4倍速）
        reasoning_effort: 'minimal',
      }),
    })

    if (!gatewayRes.ok) {
      throw new Error(`Gateway error: ${gatewayRes.status}`)
    }

    const data = await gatewayRes.json()
    const raw = data.choices?.[0]?.message?.content ?? ''

    // 応答に困った（空・空白のみ）場合は、定型質問の候補を添えて会話を続ける
    if (raw.trim().length === 0) {
      return NextResponse.json({
        content: '[normal2]うまくお答えできませんでした…。他にも聞きたいことはありますか？下から選ぶこともできますよ！',
        searchUsed: false,
        suggestions: FALLBACK_SUGGESTIONS,
      })
    }

    return NextResponse.json({ content: raw, searchUsed: needsSearch })
  } catch (error) {
    console.error('Chat API error:', error)
    return Response.json(
      {
        content: '[disappointed]うまくお答えできませんでした…。他にも聞きたいことはありますか？',
        suggestions: FALLBACK_SUGGESTIONS,
      },
      { status: 500 }
    )
  }
}
